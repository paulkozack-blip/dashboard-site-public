...
const ChartWrapper: React.FC<ChartWrapperProps> = ({ group }) => {
  const { series, volumeStack, isLoading, error, refresh } = useChartData(group);
  const tickers = useMemo(() =>
    Array.from(new Set(series.map(s => s.ticker))), [series]
  );
  ...
  const [activeIndicators, setActiveIndicators] = useState<Record<string, TickerIndicators>>({});
  ...
  useEffect(() => {
    if (tickers.length > 0) {
      console.log(`✅ [ChartWrapper] Включаем все тикеры для группы ${group}:`, tickers);
  ...
  const handleLoadIndicators = useCallback(async (ticker: string) => {
    console.log(`📥 [ChartWrapper] Загрузка индикаторов для тикера: ${ticker}`);
    
    if (!activeTickers.includes(ticker)) {
      console.warn(`⚠️ Нельзя загрузить индикаторы для неактивного тикера: ${ticker}`);
      return;
    }
    ...
    try {
      ...
      console.log(`✅ [ChartWrapper] Индикаторы для ${ticker} успешно загружены`);
    } catch (error) {
      console.error(`❌ [ChartWrapper] Ошибка загрузки индикаторов для ${ticker}:`, error);
    }
  ...
  const toggleEmaIndicator = async (period: number) => {
    ...
    if (!activeTickers.includes(currentTicker)) {
      console.warn(`⚠️ Нельзя включить индикатор для неактивного тикера: ${currentTicker}`);
      return;
    }
    
    // Проверяем, загружены ли индикаторы для этого тикера
    const hasLoadedIndicators = indicatorsHook.loadedTickers.includes(currentTicker);
    if (!hasLoadedIndicators) {
      console.log(`🔄 [ChartWrapper] Автоматическая загрузка индикаторов для ${currentTicker}`);
  ...
  const filteredVolumeStack = useMemo(() => {
    return volumeStack.map((point: VolumeStackPoint) => ({
      ...point,
      parts: point.parts.filter(part => {
        const isTickerActive = activeTickers.includes(part.ticker);
        const isVolumeActive = activeIndicators[part.ticker]?.volume !== false;
  ...
  const filteredIndicatorMap = useMemo(() => {
    ...
    Object.entries(indicatorsHook.indicatorMap).forEach(([key, indicatorData]) => {
  ...
  const handleAddFibonacciPoint = useCallback((point: { time: number; price: number }) => {
    addPoint(point);
  }, [addPoint]);
  ...
  return (
    <div className={`chart-wrapper ${fibonacciState.isDrawing ? 'drawing-fibonacci' : ''}`}>
      ...
      />
        ...
        {fibonacciState.isDrawing && (
          <div className="fibonacci-drawing-hint">
            🎯 Режим рисования Фибоначчи: кликните по графику чтобы добавить точки (2 точки)
          </div>
      )}
      ...
      {!isLoading && !error && tickers.length > 0 && activeTickers.length > 0 && (
      ...
      {!isLoading && !error && (tickers.length === 0 || activeTickers.length === 0) && (
        <div className="no-data-message">
          <p>{tickers.length === 0 ? 'Нет данных для отображения' : 'Нет активных тикеров'}</p>
        </div>
      )}

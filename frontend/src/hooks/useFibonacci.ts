...
export const useFibonacci = () => {
  ...
  const normalizeTime = useCallback(
    (timestamp: number, chartData?: Array<{ time: any }>): number => {
      ...
      if (typeof sampleTime === 'string') {
        ...
        console.log(
          '📅 График использует строковый формат времени, но точки Фибоначчи используют числа'
        );
  ...
  const addPoint = useCallback(
    (point: FibonacciPoint, chartData?: Array<{ time: any }>) => {
      setFibonacciState((prev) => {
        ...
        if (!start) {
          ...
          const newRetracement = createFibonacciRetracement(
            start,
            normalizedPoint
          );
          return {
  ...
  const createFibonacciRetracement = (
    ...
    const levels = FIBONACCI_LEVELS_CONFIG.filter((level) => level.visible).map(
      (config, index) => {
        ...
        console.log(
          `📊 [Fibonacci] Уровень ${config.label}: ${isUptrend ? lowPoint.price : highPoint.price} ${isUptrend ? '+' : '-'} ${priceDiff} * ${config.ratio} = ${price}`
        );
  ...
  const getFibonacciSeriesData = useCallback(
    (retracement: FibonacciRetracement, chartData?: Array<{ time: any }>) => {
      const data = [];

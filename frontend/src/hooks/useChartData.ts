...
export function useChartData(group: string) {
    ...
    async (seqToken: number) => {
      ...
      try {
        ...
        if (seqRef.current !== seqToken) {
          console.log(
            `🔄 [useChartData] Пропускаем устаревший результат для группы: ${group}`
          );
        ...
        console.log(
          `📈 [useChartData] Агрегация объемов для ${Object.keys(volumeData).length} тикеров`
        );
        ...
        console.error(
          `❌ [useChartData] Ошибка загрузки данных для группы ${group}:`,
          e
        );
  ...
  useEffect(() => {
    ...
    console.log(
      `🎯 [useChartData] Запуск эффекта для группы: ${group}, seq: ${currentSeq}`
    );
  ...
  const refresh = useCallback(() => {
    console.log(
      `🔄 [useChartData] Принудительное обновление для группы: ${group}`
    );
    seqRef.current += 1;

...
export function useIndicators(tickers: string[], colorMap: Record<string, string>) {
  const [settings, setSettings] = useState<IndicatorSettingsResponse | null>(null);
  const [indicatorMap, setIndicatorMap] = useState<Record<IndicatorKey, IndicatorData>>({});
  ...
  const loadTickerIndicators = useCallback(async (ticker: string) => {
    ...
    try {
      ...
      for (const period of (settings.ema_periods ?? [])) {
        ...
        try {
          const resp: IndicatorApiResponse = await apiService.getIndicatorData(ticker, 'ema', period);
          const color = colorMap[ticker] ? generateEmaColor(colorMap[ticker], period, settings.ema_periods) : '#000';
      ...
      try {
        const resp: IndicatorApiResponse = await apiService.getIndicatorData(ticker, 'rsi', settings.rsi_period);
        const color = colorMap[ticker] ? generateRsiColor(colorMap[ticker]) : '#888';
        const ind = adaptIndicatorResponse(resp, color, 'rsi', settings.rsi_period);
  ...
  const loadIndicators = useCallback(async (tickersToLoad: string[]) => {
    if (!settings) {
      await fetchSettings();
    }
    ...
    try {
      for (const t of tickers) {
        ...
        for (const period of (settings.ema_periods ?? [])) {
          ...
          try {
            const resp: IndicatorApiResponse = await apiService.getIndicatorData(t, 'ema', period);
            const color = colorMap[t] ? generateEmaColor(colorMap[t], period, settings.ema_periods) : '#000';
            const ind = adaptIndicatorResponse(resp, color, 'ema', period);
            out[key] = ind;
          } catch (e) {
        ...
        try {
          const resp: IndicatorApiResponse = await apiService.getIndicatorData(t, 'rsi', settings.rsi_period);
